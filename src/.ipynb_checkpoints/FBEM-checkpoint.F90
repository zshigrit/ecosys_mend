...
contains
    ...
    subroutine can_photosyn(gpp)
      ! will use name list for key parameters
        ...
        fp = './src/photosyn/examples/biocon/input/'
        filename_full = trim(fp)//"tair_rh_rad_hourly.in"
        open(12,file=filename_full)!,status='old',access ='sequential',form='formatted')
        open(13,file='./src/photosyn/df_swc_daily.in')
        filename_full = trim(fp)//"df_lai_daily.in"
        open(14,file=filename_full)
        read(12,*) header
        read(13,*) header
        read(14,*) header
